...
| 复制配置预览 | **电力节点同款：只画【计划↔计划】**——悬停幽灵走 drawPlanConfig，入队/原理图粘贴/拖线走 drawPlanConfigTop；对配置偏移在同批计划中找落点：兄弟中枢粉色、其它可连方块物流色；距离校验（范围内才画）防莫名虚线；**绝不画向实际建筑**；固定层级 z=96（flyingUnitLow 之上、bullet 之下，拖动全程不被遮挡） |
