...
- [ ] **满连接的中枢**点击同网/他网建筑一律不动（容量闸门先于归属裁决，防剥离成孤儿链接）
- [ ] 中枢↔中枢互联不受同网排除限制
...
| **放置侧连接预览** | 放置非中枢可连建筑时：最近中枢→幽灵实线激光 + 幽灵蓝框 + 中枢范围虚线圈（`Layer.plans+4`，同挂点 drawOver 绘制）；「最近中枢」与建造完成事件同口径；无中枢覆盖不标记＝放下后不会自动接入 |
| 预览/单击三色标记 | **已/将直连=蓝** Pal.place；**网络内未直连=紫** Pal.reactorPurple（预览时=即将连入的网络）；**可新建（网络外）=绿** Pal.heal——预览与单击同口径 |
| 复制配置预览 | **电力节点同款：只画【计划↔计划】**——悬停幽灵走 drawPlanConfig，入队/原理图粘贴/拖线走 drawPlanConfigTop；对配置偏移在同批计划中找落点：兄弟中枢粉色、其它可连方块物流色；**实线激光与常驻连线完全同款**（贴图/端点公式/透明度）；距离校验（范围内才画）防莫名连线；**绝不画向实际建筑**；层级显式抬升 `max(当前层+1, Layer.plans+4)`——严格压过同层后绘制的计划幽灵，拖动全程不被遮挡 |
...
| 放置连接顺序 | **复制放置不补连**：placeEnded 检测显式 Point2[] 配置或 configured 已写入任意连接/挂起 → 跳过 autoConnectNearby，拓扑完全由复制模式 + 挂起队列决定（所见即所得，不接入周边原网络、不占配额饿死偏后挂起目标）；无配置的新枢才用剩余容量像电力节点一样补连周围 |
| 连接单一归属 | 普通目标任一时刻只被一个中枢服务：认领路径（Point2[] 复制配置 / Integer 单击·双击自动连·挂起补连）统一经 `stealFromOtherHubs` 从其它全部中枢的 links/hubLinks/pendingLinks 抢回；中枢↔中枢粉色连接不受此约束 |
