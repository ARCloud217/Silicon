...
| **放置侧连接预览** | 放置非中枢可连建筑时：范围内最近中枢→幽灵实线激光 + 幽灵蓝框 + 中枢范围虚线圈（`Layer.overlayUI`，drawOver 覆盖层绘制）；与建造完成事件【完全同口径】：①位置已有归属→不标记 ②胜者=最近可连枢（满员不跳次近）③胜者满员→不标记（实际不会接入） |
| 预览/单击三色标记 | **已/将直连=蓝** Pal.place；**网络内未直连=紫** Pal.reactorPurple（预览时=即将连入的网络）；**可新建（网络外）=绿** Pal.heal——预览与单击同口径 |
| 复制配置预览 | **电力节点同款钩子（仅此处能看到同批计划），层级固定抬升 `Layer.overlayUI`(120)**——高于一切世界几何（方块≤40/常驻连线88/子弹100/特效110/飞行单位115），同帧任何后绘制精灵都无法遮挡；只画【计划↔计划】：兄弟中枢粉色、其它可连方块物流色；距离校验（范围内才画）防莫名连线；**绝不画向实际建筑**；调试日志记录实际绘制 z 与环境 z |
