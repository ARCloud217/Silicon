...
| （待定版） | **复制对齐电力节点**：①开启 `saveConfig=true`——反编译确认 v159 原版闸门：悬停幽灵（InputHandler.drawPlan）与拖线/单击放置计划（Block.nextConfig）只在 saveConfig=true 时携带拾取的 lastConfig，此前复制配置到不了放置流程导致「复制时不显示连接、放置后无链接」；②新增 drawPlan 钩子——悬停幽灵/拖线计划/入队计划（含原理图粘贴）统一画出将建立的连接虚线（中枢间粉色、其余物流色）；③预览层级 Layer.plans-5（85→80）：规划幽灵之下、电力线之上（「其它方块后、电力节点前」）；④开发版本号加 `-dev` 后缀避免与上游定版同号 |
