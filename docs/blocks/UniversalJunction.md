...
|------|------|
| a0.12.1.0 | 初始创建 |
| a0.12.1.0 | （待定版）UI 优化：同值折叠增量更新消除拖动闪烁、缓冲区溢出修正、dirName 缓存、tapOpen 跨方向保持、确认弹窗、禁用方向红色标记、禁用方向连接边视觉反馈 |
| a0.12.1.0 | （待定版）新版 UI 重构：面板顺序重排（全局→覆盖→模板）、全局滑块增量更新不重建覆盖层、快捷按钮精简（均分/清零）、模板列表始终可见、取消所有覆盖按钮 |
