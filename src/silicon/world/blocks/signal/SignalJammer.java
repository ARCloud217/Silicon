...
/**
 * 信号干扰器（1×1）：在指定信道（0~4，或全信道 ALL=-1）发射压制噪声。
 * 干扰半径内同信道接收器无法解调信号（hasSignal=false）。
 * 用于破坏敌方/本队通信、保护频段。
 */
public class SignalJammer extends Block {
    ...
    public SignalJammer(String name) {
        ...
        update = true;
        configurable = true;
        config(Integer.class, (SignalJammerBuild b, Integer v) -> b.jamChannel = Math.max(-1, Math.min(SignalNet.CHANNEL_MAX, v)));
    }
    ...
    public static boolean jammed(Team team, int channel, float wx, float wy) {
        ...
        for (SignalJammerBuild jb : allJammers(team)) {
            ...
            if (arc.math.Mathf.dst(jb.x, jb.y, wx, wy) <= rangePx) return true;
        }
        return false;
    }
    ...
    public class SignalJammerBuild extends Building {
        ...
        public void buildConfiguration(Table table) {
            ...
            group.add(allBtn);
            table.add(allBtn).size(72f, 40f).pad(2f);
        ...
        public void display(Table table) {
            super.display(table);
            table.row();
            table.add(Core.bundle.format("block.silicon-signal-jammer.channel.current",
